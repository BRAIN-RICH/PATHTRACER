...
Settings loadSettings()
{
    ...
    settings.renderType = 2;//设置初始渲染模式
    settings.minDepth = 1;//最小弹射次数
    settings.maxDepth = 50;//最大弹射次数
    settings.gamma = 1.0f;
    settings.backgroundLight = Vec3f{0.0f, 0.0f, 0.0f};//背景光
    settings.stepSize = 0.1f;
    settings.useBox = false;//是否使用包围盒

    settings.octreeLevels = 5;//八叉树层数
    ...
    settings.maxOpacity.resize(settings.brackets.size() - 1);//最大透明度
    settings.minStepSize.resize(settings.brackets.size() - 1);//最小步长
...
std::shared_ptr<Material> parseMaterial(std::ifstream &fin, std::shared_ptr<Texture> const& texture)
{
	...
	//材质模型，lamber ， phong ， glossy  , Mirror
    if (type == "Lambert")
    {
...
void loadBrain(scg::Volume& volume, scg::Volume& temp, Scene &scene, scg::Settings &settings,std::string filepath)
{
    ...
    for (int x = 0; x < _file.size(); ++x)
    {
        ...
        {
            int width = TinyTIFFReader_getWidth(tiffr);//获取宽度
            int height = TinyTIFFReader_getHeight(tiffr);//获取高度
            uint16_t* image = (uint16_t*)calloc((size_t)width * height, sizeof(uint16_t));//申请内存空间，并赋值为0.
...
{
    char filename[50] = "D:pathtracer/data/StanfordBunny/";
