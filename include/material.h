...
{
public:
    virtual Vec3f evaluate(ScatterEvent const& interaction) const = 0;//如何计算
    virtual void sample(ScatterEvent &interaction, Sampler &sampler) const = 0;
    virtual float pdf(ScatterEvent const& interaction) const = 0;//概率密度函数

    virtual BSDFLobe getSupportedLobes(Vec2f const&) const = 0;//旋转支持的bsdf函数
    virtual std::shared_ptr<Light> getLight(Vec2f const&) const//获取光线
    {
        return nullptr;
    }
};
