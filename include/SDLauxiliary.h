...
screen* InitializeSDL(int width,int height, bool fullscreen)
{
    ...
    flags = SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC;//加速方法
...
void PutPixelSDL(screen* s, int x, int y, scg::Vec3f colour)
{
	...
	//uint32_t 32位的无符号整型
    uint32_t alpha = 255;
    uint32_t r = uint32_t( scg::clamp( 255*colour.r, 0.f, 255.f ) );//clamp限定函数，限定在0~255之间
