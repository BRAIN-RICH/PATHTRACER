...
private:
    ...
    std::uniform_real_distribution<float> distribution;//用于均匀分布中生成随机浮点数
...
public:
    ...
    float nextFloat()
    {
        return distribution(generator);//返回以generator为种子的随机数
    }

    unsigned int nextDiscrete(unsigned int range)
    {
        return (unsigned int)/*std::floor*/(nextFloat() * range);//返回随机数*输入范围参数
    }
};
